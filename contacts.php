...
foreach($arr as $data) {
  if((!(in_array($data['sendId'], $arrUsed)) && $data['sendId'] != $userId )) {
    if($data['sendId'] == $sendId) {
      ...
      <input type='submit' style='pointer-events:none;background-color:#A6192E;color:white;border-radius:0;margin:0;' class='contact' value='{$data['email']}'>
      </form>";
      array_push($arrUsed, $data['sendId']);
    } else {
    echo "<form action='chat.php' method='post' >
    <input type='hidden' name='sendId' value='{$data['sendId']}'>
    <input type='submit' style='background-color:lightgrey;color:black;border-bottom:1px solid grey;border-radius:0;margin:0;' class='contact' value='{$data['email']}'>
    </form>";
    array_push($arrUsed, $data['sendId']);
    }
  }
